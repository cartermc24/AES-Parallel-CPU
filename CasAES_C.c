...
//Change this variable to adjust the number of CPU threads that run at the same time
//Set for the number of cores the CPU has
...
void ark(uint8_t* state, int strD, uint32_t* eK)
{
    union {
        uint32_t word;
        uint8_t bytes[4];
...
void* mainCypher(void *tcp)
{
    ...
    for (int i = 1; i < 14; i++)
    {
        ...
        sr(*tc->state);
        mc(*tc->state);
        ark(*tc->state, i*Nb, ek);
    }
    
    sb_st(*tc->state);
    sr(*tc->state);
    ark(*tc->state, Nr*Nb, ek);
...
{
    printf("CasAES_C Hyperthreaded AES-256 Encryption - compiled 01/14/2014 Rev. 2\nCarter McCardwell, Northeastern University NUCAR - http://coe.neu.edu/~cmccardw - mccardwell.net\nRunning with %i threads - Please Wait...\n", RUNNING_THREADS);
    ...
    uint8_t states[RUNNING_THREADS][16];
